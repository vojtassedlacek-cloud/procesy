...
/* ===================================================================
   API klíč Power-Upu z trello.com/power-ups/admin (záložka API klíč).
   Není to heslo, jen identifikátor aplikace — může být klidně veřejně.
   Měnit ho stačí, když v adminu vygeneruješ nový.
   Pozor: v adminu musí být mezi povolenými původy adresa, ze které
   Power-Up běží — https://vojtassedlacek-cloud.github.io
   =================================================================== */
...
var state = {
  ...
  lists: [],            // seznamy boardu
  boardLabels: [],      // štítky boardu pro editační panel
  boardMembers: [],     // členové boardu
  maps: [],             // [{ id, name }]
  mapId: null,          // otevřená mapa
  saveTimer: null,
  texts: [],            // vlastní textové uzly mimo Trello
  frames: [],           // ohraničené oblasti (jako sekce v Miro)
  selectedSet: {},      // vícenásobný výběr { idUzlu: true }
  hidden: {},           // uzly skryté v mapě (v Trellu zůstávají)
  collapsed: {},        // sbalené větve
  cloaked: {},          // co je zrovna schované pod sbalenou větví
  boxSel: null,
  selectedFrame: null,
  frameDrag: null
};
...
var el = {
  ...
  ceMembers:document.getElementById('ce-members'),
  ...
  btnHidden:document.getElementById('btn-hidden'),
  hb:       document.getElementById('hiddenbox'),
  hp:       document.getElementById('helpbox'),
  zoomVal:  document.getElementById('zoom-val'),
  hbList:   document.getElementById('hb-list'),
  selbox:   document.getElementById('selbox'),
  nc:       document.getElementById('newcard'),
  ncName:   document.getElementById('nc-name'),
  ncList:   document.getElementById('nc-list'),
  ncDue:    document.getElementById('nc-due'),
  ncStatus: document.getElementById('nc-status')
};
...
function loadData() {
  ...
  return Promise.all([
    t.board('id', 'name', 'labels', 'members'),
    t.lists('id', 'name'),
    t.cards('id', 'name', 'idList', 'shortLink', 'url', 'labels', 'members', 'badges', 'closed', 'due', 'dueComplete')
  ]).then(function (res) {
    state.board = res[0];
    state.boardLabels = (res[0] && res[0].labels) || [];
    state.boardMembers = (res[0] && res[0].members) || [];
    state.listName = {};
    state.lists = res[1] || [];
    state.lists.forEach(function (l) { state.listName[l.id] = l.name; });
    ...
    cards.forEach(function (c) {
      ...
      var top = labelsHeight(c) + PAD_TOP;
...
function saveMapData() {
  ...
  var data = {
    ...
    hd: Object.keys(state.hidden),
    cl: Object.keys(state.collapsed),
...
function openMap(id) {
  ...
  return loadMapData(id).then(function (data) {
    ...
    state.hidden = {};
    (data.hd || []).forEach(function (id) { state.hidden[id] = true; });
    state.collapsed = {};
    (data.cl || []).forEach(function (id) { state.collapsed[id] = true; });
...
// Duplikace kopíruje uložená data mapy pod nové id — karty v Trellu
// zůstávají tytéž, kopíruje se jen rozvržení, spojnice, barvy a texty.
function duplicateMap(id) {
  var meta = state.maps.filter(function (m) { return m.id === id; })[0];
  if (!meta) return;

  var name = window.prompt('Název kopie:', meta.name + ' – kopie');
  if (name === null) return;
  name = (name || '').trim() || (meta.name + ' – kopie');

  setStatus('Kopíruju mapu…');

  loadMapData(id).then(function (data) {
    // textovým uzlům dáme nová id, ať se kopie s originálem nepletou
    var idMap = {};
    (data.t || []).forEach(function (x, i) {
      idMap[x.i] = 'x' + Date.now().toString(36) + i.toString(36);
      x.i = idMap[x.i];
    });
    (data.f || []).forEach(function (f, i) {
      f.i = 'f' + Date.now().toString(36) + i.toString(36);
    });
    (data.e || []).forEach(function (a) {
      if (idMap[a[0]]) a[0] = idMap[a[0]];
      if (idMap[a[2]]) a[2] = idMap[a[2]];
    });
    var pos = {};
    Object.keys(data.p || {}).forEach(function (k) {
      pos[idMap[k] || k] = data.p[k];
    });
    data.p = pos;
    var cols = {};
    Object.keys(data.c || {}).forEach(function (k) {
      cols[idMap[k] || k] = data.c[k];
    });
    data.c = cols;
    data.hd = (data.hd || []).map(function (k) { return idMap[k] || k; });

    var newId = 'm' + Date.now().toString(36);
    var raw = JSON.stringify(data);
    var jobs = [];
    for (var i = 0; i < 10; i++) {
      var part = raw.slice(i * 3400, (i + 1) * 3400);
      if (part) jobs.push(t.set('board', 'shared', 'm_' + newId + '_' + i, part));
    }

    return Promise.all(jobs).then(function () {
      state.maps.push({ id: newId, name: name });
      return saveMapList();
    }).then(function () {
      renderMapList();
      setStatus('');
      t.alert({ message: 'Kopie „' + name + '“ je hotová.', duration: 4 });
    });
  }).catch(function (err) {
    console.error(err);
    setStatus('');
    t.alert({ message: 'Kopii se nepodařilo vytvořit.', duration: 6 });
  });
}
...
function openCardEditor(n) {
  ...
  ceMemberIds = (n.card.members || []).map(function (m) { return m.id; }).filter(Boolean);
  ...
  // členové boardu
  el.ceMembers.innerHTML = '';
  if (!state.boardMembers.length) {
    var none = document.createElement('span');
    none.className = 'ce-note';
    none.textContent = 'Board nemá žádné členy.';
    el.ceMembers.appendChild(none);
  }
  state.boardMembers.forEach(function (m) {
    var b = document.createElement('button');
    b.className = 'ce-mem' + (ceMemberIds.indexOf(m.id) >= 0 ? ' on' : '');
    b.innerHTML = '<i></i><span></span>';
    b.querySelector('i').textContent = initialsOf(m);
    b.querySelector('span').textContent = m.fullName || m.username || '';
    b.addEventListener('click', function () {
      var i = ceMemberIds.indexOf(m.id);
      if (i >= 0) { ceMemberIds.splice(i, 1); b.classList.remove('on'); }
      else { ceMemberIds.push(m.id); b.classList.add('on'); }
    });
    el.ceMembers.appendChild(b);
  });
...
function saveCard() {
  ...
  ensureToken().then(function (token) {
    ...
    // API vrací jen id členů, doplníme je z členů boardu kvůli iniciálám
    n.card.members = (card.idMembers || []).map(function (id) {
      return state.boardMembers.filter(function (m) { return m.id === id; })[0] || { id: id };
    });
    n.name = card.name;
    n.lines = wrapText(card.name, 26, MAX_LINES);
    n.hasLabels = !!(n.card.labels && n.card.labels.length);
    n.top = labelsHeight(n.card) + PAD_TOP;
...
function childrenOf(id) {
  var out = [];
  state.edges.forEach(function (e) { if (e.f === id) out.push(e.t); });
  return out;
}

// Projde sbalené uzly a označí všechno, co pod nimi visí, jako schované.
function recomputeCloaked() {
  state.cloaked = {};
  var seen = {};
  ...
  function walk(id) {
    childrenOf(id).forEach(function (cid) {
      if (seen[cid]) return;
      seen[cid] = true;
      state.cloaked[cid] = true;
      walk(cid);
    });
  }

  Object.keys(state.collapsed).forEach(function (id) {
    if (state.nodeById[id]) walk(id);
  });
}

function isVisible(id) { return !state.hidden[id] && !state.cloaked[id]; }
...
/* --- skrývání uzlů v mapě (Trello zůstává nedotčené) ----------------- */

function hideSelected() {
  var ids = Object.keys(state.selectedSet);
  if (!ids.length) return;
  ids.forEach(function (id) { state.hidden[id] = true; });
  clearSelection();
  closeCardEditor();
  closeTextEditor();
  render();
  scheduleSave();
  t.alert({
    message: ids.length === 1 ? 'Uzel je skrytý jen v mapě, v Trellu zůstal.'
                              : ids.length + ' uzlů je skrytých jen v mapě, v Trellu zůstaly.',
    duration: 4
  });
}
...
function updateHiddenButton() {
  ...
  el.btnHidden.textContent = 'Skryté (' + n + ')';
}

function openHiddenBox() {
  renderHiddenList();
  el.hb.classList.add('show');
}

function closeHiddenBox() { el.hb.classList.remove('show'); }

function renderHiddenList() {
  el.hbList.innerHTML = '';
  var ids = Object.keys(state.hidden);
  if (!ids.length) {
    var p = document.createElement('div');
    p.className = 'ce-note2';
    p.textContent = 'Nic není skryté.';
    el.hbList.appendChild(p);
    return;
  }
  ids.forEach(function (id) {
    ...
    label.textContent = n ? (n.name || '(bez názvu)') : '(uzel už neexistuje)';
    row.appendChild(label);

    var b = document.createElement('button');
    b.textContent = 'Vrátit';
    b.addEventListener('click', function () { unhide(id); });
    row.appendChild(b);

    el.hbList.appendChild(row);
  });
}

/* --- nový úkol do Trella --------------------------------------------- */

var ncFromText = null;   // textový uzel, který se převádí na kartu

function openNewCard(fromNode) {
  if (!state.mapId) { t.alert({ message: 'Nejdřív otevři nebo založ mapu.', duration: 5 }); return; }
  if (!keyReady()) { t.alert({ message: 'Nejdřív vlož API klíč do mapa.js.', duration: 6 }); return; }

  ncFromText = fromNode || null;
  ...
  state.lists.forEach(function (l) {
    ...
    el.ncList.appendChild(o);
  });

  el.ncName.value = ncFromText ? (ncFromText.ref.text || '') : '';
  el.ncDue.value = '';
  el.ncStatus.textContent = '';
  el.nc.classList.add('show');
  el.ncName.focus();
}

function closeNewCard() {
  el.nc.classList.remove('show');
  ncFromText = null;
}

function createCard() {
  var name = el.ncName.value.trim();
  if (!name) { el.ncName.focus(); return; }
  var idList = el.ncList.value;
  if (!idList) { t.alert({ message: 'Board nemá žádný seznam, kam kartu založit.', duration: 5 }); return; }

  var body = { name: name, idList: idList, pos: 'bottom' };
  if (el.ncDue.value) body.due = new Date(el.ncDue.value + 'T12:00:00').toISOString();

  el.ncStatus.textContent = 'Zakládám…';

  ensureToken().then(function (token) {
    return fetch('https://api.trello.com/1/cards?key=' + encodeURIComponent(APP_KEY) +
    ...
    if (!res.ok) throw new Error('Trello kartu nevytvořilo (' + res.status + ')');
    return res.json();
  }).then(function (card) {
    var src = ncFromText;
    if (src) {
      // textový uzel se promění v kartu — pozici i spojnice si nechá
      var newId = 'c:' + card.id;
      state.edges.forEach(function (e) {
        if (e.f === src.id) e.f = newId;
        if (e.t === src.id) e.t = newId;
      });
      if (state.colors[src.id]) { state.colors[newId] = state.colors[src.id]; delete state.colors[src.id]; }
      var px = src.x, py = src.y;
      state.texts = state.texts.filter(function (x) { return x.id !== src.id; });
      rebuildTextNodes();
      addCardNode(card, px, py);
      closeTextEditor();
    } else {
      addCardNode(card);
    }
    el.ncStatus.textContent = '';
    closeNewCard();
    t.alert({ message: 'Úkol „' + card.name + '“ je v Trellu i v mapě.', duration: 4 });
  }).catch(function (err) {
    console.error(err);
    el.ncStatus.textContent = '';
    t.alert({ message: 'Nepovedlo se: ' + (err && err.message ? err.message : err), duration: 7 });
  });
}

// Novou kartu položíme doprostřed pohledu, ať ji uživatel hned vidí.
function addCardNode(card, atX, atY) {
  var rect = el.wrap.getBoundingClientRect();
  var cx = (rect.width / 2 - state.view.x) / state.view.k;
  var cy = (rect.height / 2 - state.view.y) / state.view.k;

  var lines = wrapText(card.name, 30, MAX_LINES);
  var top = labelsHeight(card) + PAD_TOP;
  var n = {
    id: 'c:' + card.id, kind: 'card', listId: card.idList, name: card.name, card: card,
    lines: lines, top: top, hasLabels: !!(card.labels && card.labels.length),
    h: top + lines.length * LINE_H + (badgeText(card) ? 16 : 0) + PAD_BOT,
    x: (typeof atX === 'number') ? atX : Math.round((cx - NODE_W / 2) / GRID) * GRID,
    y: (typeof atY === 'number') ? atY : Math.round(cy / GRID) * GRID
  };
  state.nodes.push(n);
...
/* --- rychlé skládání klávesnicí -------------------------------------- */

// Tab přidá navázaný uzel vedle vybraného, Enter sourozence pod ním.
function quickNode(mode, fromNode) {
  if (!state.mapId) return;
  var src = fromNode || state.nodeById[state.selected];
  if (!src) return;

  var x = (mode === 'child') ? src.x + NODE_W + COL_GAP : src.x;
  var y = (mode === 'child') ? src.y : src.y + src.h + ROW_GAP;
  if (mode === 'below') { x = src.x; y = src.y + src.h + ROW_GAP; }

  // aby nové uzly nepadaly na sebe, posuneme se dolů, dokud je místo obsazené
  var guard = 0;
  while (guard++ < 60 && state.nodes.some(function (m) {
    return isVisible(m.id) && Math.abs(m.x - x) < 20 && Math.abs(m.y - y) < 20;
  })) { y += ROW_GAP + 40; }
  ...
  var item = {
    ...
    text: '', heading: false, size: 'm', note: ''
  };
  state.texts.push(item);
  rebuildTextNodes();

  if (mode === 'child') {
    state.edges.push({ f: src.id, fh: 'r', t: item.id, th: 'l' });
  } else if (mode === 'below') {
    state.edges.push({ f: src.id, fh: 'b', t: item.id, th: 'tp' });
  } else {
    // sourozenec dostane stejné rodiče jako vybraný uzel
    state.edges.filter(function (e) { return e.t === src.id; }).forEach(function (e) {
      state.edges.push({ f: e.f, fh: e.fh, t: item.id, th: e.th });
    });
  }

  state.selectedSet = {};
  state.selectedSet[item.id] = true;
  state.selected = item.id;
  render();
  scheduleSave();
  startEdit(state.nodeById[item.id]);
}
...
function nudgeSelection(dx, dy) {
  ...
  ids.forEach(function (id) {
    ...
    if (!n) return;
    n.x += dx; n.y += dy;
    if (n.kind === 'text' && n.ref) { n.ref.x = n.x; n.ref.y = n.y; }
  });
  render();
  scheduleSave();
}
...
function renderMapList() {
  ...
  if (!state.maps.length) {
    ...
    note.textContent = 'Zatím tu žádná mapa není — vyber si šablonu níže a začni.';
  ...
  state.maps.forEach(function (m) {
    ...
    ico.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke-width="2" ' +
      'stroke-linecap="round" stroke-linejoin="round">' +
      '<rect x="1" y="9.5" width="7" height="5" rx="1.5"/>' +
      '<rect x="16" y="3" width="7" height="5" rx="1.5"/>' +
      '<rect x="16" y="16" width="7" height="5" rx="1.5"/>' +
      '<path d="M8 12h3.5M11.5 12V5.5H16M11.5 12v6.5H16"/></svg>';
    row.appendChild(ico);
    ...
    [['Otevřít', 'main', function () { openMap(m.id); }],
     ['Duplikovat', '', function () { duplicateMap(m.id); }],
     ['Přejmenovat', '', function () { renameMap(m.id); }],
     ['Smazat', 'danger', function () { deleteMap(m.id); }]].forEach(function (b) {
      var btn = document.createElement('button');
      btn.textContent = b[0];
      if (b[1]) btn.className = b[1];
      btn.addEventListener('click', b[2]);
      row.appendChild(btn);
    });

    // dvojklik na řádek mapu rovnou otevře
    row.addEventListener('dblclick', function () { openMap(m.id); });
...
function setColor(key) {
  ...
  if (!ids.length) return;
  ids.forEach(function (id) {
    if (key) state.colors[id] = key;
    else delete state.colors[id];
  });
  render();
  saveColors();
}
...
function render() {
  ...
  state.edges.forEach(function (e) {
    if (!isVisible(e.f) || !isVisible(e.t)) return;
    drawEdge(e);
  });
  state.nodes.forEach(function (n) {
...
function nodeEl(n) {
  ...
  if (n.kind === 'card') {
    // štítky i s názvem, zalomené do řádků
    if (n.hasLabels) {
      labelChips(n).forEach(function (ch) {
        g.appendChild(svgEl('rect', {
          x: ch.x, y: ch.y, width: ch.w, height: 17, rx: 4, ry: 4, fill: ch.color
        }));
        if (ch.text) {
          var lt = svgEl('text', {
            x: ch.x + 7, y: ch.y + 12.5, 'class': 'node-chip', fill: '#fff'
          });
          lt.textContent = ch.text;
          g.appendChild(lt);
        }
      });
    }
    ...
    if (lname) {
      var lt = svgEl('text', { x: 14, y: labelsHeight(n.card) + 17, 'class': 'node-list' });
  ...
  if (n.kind === 'card') {
    ...
    var mem = n.card.members || [];
    mem.slice(0, 4).forEach(function (m, i) {
      var cx = NODE_W - 20 - i * 24;
      var mg = svgEl('g', { transform: 'translate(' + cx + ',' + (n.h - 18) + ')' });
      mg.appendChild(svgEl('circle', { cx: 0, cy: 0, r: 10, fill: BRAND.dark }));
      var it = svgEl('text', { x: 0, y: 3.4, 'class': 'node-ava', 'text-anchor': 'middle' });
      it.textContent = initialsOf(m);
      mg.appendChild(it);
      var ttl = document.createElementNS('http://www.w3.org/2000/svg', 'title');
      ttl.textContent = m.fullName || m.username || '';
      mg.appendChild(ttl);
      g.appendChild(mg);
    });
  }

  // pluska pro rychlé navázání dalšího uzlu
  [['child', NODE_W + 20, n.h / 2, 'Navázat vpravo'],
   ['below', NODE_W / 2, n.h + 20, 'Navázat dolů']].forEach(function (a) {
    var ag = svgEl('g', { 'class': 'addbtn', transform: 'translate(' + a[1] + ',' + a[2] + ')' });
    ag.appendChild(svgEl('circle', { cx: 0, cy: 0, r: 11 }));
    ag.appendChild(svgEl('path', { d: 'M-5,0 H5 M0,-5 V5' }));
    var ttl = document.createElementNS('http://www.w3.org/2000/svg', 'title');
    ttl.textContent = a[3];
    ag.appendChild(ttl);
    ag.addEventListener('mousedown', function (ev) { ev.stopPropagation(); ev.preventDefault(); });
  ...
  // kolečko, kterým se sbalí celá větev pod uzlem
  var kids = countBranch(n.id);
  if (kids > 0) {
    var cg = svgEl('g', { 'class': 'collapse-btn', transform: 'translate(' + (NODE_W - 18) + ',' + (n.h - 1) + ')' });
    cg.appendChild(svgEl('circle', { cx: 0, cy: 0, r: 11 }));
    var num = svgEl('text', { x: 0, y: 3.6, 'class': 'collapse-num', 'text-anchor': 'middle' });
    num.textContent = state.collapsed[n.id] ? kids : '−';
    cg.appendChild(num);
    cg.addEventListener('mousedown', function (ev) { ev.stopPropagation(); ev.preventDefault(); });
...
// Rozvrhne štítky do řádků a vrátí jejich souřadnice. Šířku textu
// odhadujeme z počtu znaků — v SVG se měřit předem nedá.
function labelChips(n) {
  var out = [], x = 14, y = 8, row = 0;
  (n.card.labels || []).forEach(function (lab) {
    var text = (lab.name || '').trim();
    var w = text ? Math.min(text.length * 6.4 + 14, NODE_W - 28) : 26;
    if (x + w > NODE_W - 10 && x > 14) {
      row++;
      if (row > 1) return;         // víc než dva řádky štítků nekreslíme
      x = 14; y += 21;
    }
    out.push({ x: x, y: y, w: w, text: text, color: LABEL_COLORS[lab.color] || '#b3bac5' });
    x += w + 5;
  });
  return out;
}

function labelsHeight(card) {
  if (!card.labels || !card.labels.length) return 0;
  var x = 14, rows = 1;
  card.labels.forEach(function (lab) {
    var text = (lab.name || '').trim();
    var w = text ? Math.min(text.length * 6.4 + 14, NODE_W - 28) : 26;
    if (x + w > NODE_W - 10 && x > 14) { rows++; x = 14; }
    x += w + 5;
  });
  return Math.min(rows, 2) * 21 + 4;
}
...
function startNodeDrag(n, ev) {
  ...
  if (ev.ctrlKey || ev.metaKey || ev.shiftKey) {
    if (state.selectedSet[n.id]) delete state.selectedSet[n.id];
    else state.selectedSet[n.id] = true;
  } else if (!state.selectedSet[n.id]) {
    state.selectedSet = {};
    state.selectedSet[n.id] = true;
  }

  state.selected = n.id;
  state.selectedFrame = null;

  // táhne se celý výběr, ne jen uzel pod kurzorem
  var group = Object.keys(state.selectedSet).map(function (id) {
    var m = state.nodeById[id];
    if (!m) return null;
    return { node: m, dx: p.x - m.x, dy: p.y - m.y };
  }).filter(Boolean);

  state.drag = { node: n, dx: p.x - n.x, dy: p.y - n.y, moved: false, group: group };
...
function highlightSelection() {
  Array.prototype.forEach.call(el.nodes.querySelectorAll('.node'), function (g) {
    g.classList.toggle('selected', !!state.selectedSet[g.getAttribute('data-id')]);
  });
}
...
(function initPointer() {
  ...
  el.wrap.addEventListener('mousedown', function (e) {
    ...
    if (e.shiftKey) {
      var sp = toCanvas(e);
      state.boxSel = { x0: sp.x, y0: sp.y, x1: sp.x, y1: sp.y };
      el.selbox.style.display = 'block';
      return;
    }
  ...
  window.addEventListener('mousemove', function (e) {
    ...
    if (state.boxSel) {
      var bp = toCanvas(e);
      state.boxSel.x1 = bp.x;
      state.boxSel.y1 = bp.y;
      var bx = Math.min(state.boxSel.x0, bp.x), by = Math.min(state.boxSel.y0, bp.y);
      el.selbox.setAttribute('x', bx);
      el.selbox.setAttribute('y', by);
      el.selbox.setAttribute('width', Math.abs(bp.x - state.boxSel.x0));
      el.selbox.setAttribute('height', Math.abs(bp.y - state.boxSel.y0));
      return;
    }
    ...
    if (state.drag) {
      var q = toCanvas(e);
      var moved = false;
      (state.drag.group || [{ node: state.drag.node, dx: state.drag.dx, dy: state.drag.dy }])
        .forEach(function (it) {
          var nx = Math.round((q.x - it.dx) / GRID) * GRID;
          var ny = Math.round((q.y - it.dy) / GRID) * GRID;
          if (nx !== it.node.x || ny !== it.node.y) moved = true;
          it.node.x = nx;
          it.node.y = ny;
          if (it.node.kind === 'text' && it.node.ref) { it.node.ref.x = nx; it.node.ref.y = ny; }
          var g = el.nodes.querySelector('[data-id="' + it.node.id + '"]');
          if (g) g.setAttribute('transform', 'translate(' + nx + ',' + ny + ')');
        });
      if (moved) state.drag.moved = true;
      el.edges.innerHTML = '';
      state.edges.forEach(function (ed) {
        if (!isVisible(ed.f) || !isVisible(ed.t)) return;
        drawEdge(ed);
      });
      positionPalette();
      return;
  ...
  window.addEventListener('mouseup', function (e) {
    if (state.boxSel) {
      var b = state.boxSel;
      var x0 = Math.min(b.x0, b.x1), x1 = Math.max(b.x0, b.x1);
      var y0 = Math.min(b.y0, b.y1), y1 = Math.max(b.y0, b.y1);
      state.selectedSet = {};
      state.nodes.forEach(function (n) {
        if (state.hidden[n.id]) return;
        if (n.x + NODE_W > x0 && n.x < x1 && n.y + n.h > y0 && n.y < y1) state.selectedSet[n.id] = true;
      });
      var ids = Object.keys(state.selectedSet);
      state.selected = ids.length ? ids[0] : null;
      el.btnDelete.disabled = !ids.length;
      el.btnHide.disabled = !ids.length;
      setPaletteEnabled(!!ids.length);
      highlightSelection();
      el.selbox.style.display = 'none';
      state.boxSel = null;
      return;
    }
  ...
  window.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Escape') {
      clearLink(); closeCardEditor(); closeTextEditor(); closeNewCard();
      closeHiddenBox(); closeHelp();
    }

    if (typing) return;

    if (e.key === 'Tab')   { e.preventDefault(); quickNode('child'); return; }
    if (e.key === 'Enter') { e.preventDefault(); quickNode('sibling'); return; }
    if (e.key === 'F2' && state.selected) {
      e.preventDefault();
      var sn = state.nodeById[state.selected];
      if (sn && sn.kind === 'text') openTextEditor(sn);
      else if (sn && sn.kind === 'card') openCardEditor(sn);
      return;
    }
    if (e.key === '+' || e.key === '=') { e.preventDefault(); zoomBy(1.15); return; }
    if (e.key === '-' || e.key === '_') { e.preventDefault(); zoomBy(1 / 1.15); return; }
    if (e.key === '0') { e.preventDefault(); fitToScreen(); updateZoomLabel(); return; }

    var step = e.shiftKey ? GRID * 5 : GRID;
    ...
    if (e.key === 'ArrowUp')    { e.preventDefault(); nudgeSelection(0, -step); return; }
    if (e.key === 'ArrowDown')  { e.preventDefault(); nudgeSelection(0, step); return; }
...
function updateZoomLabel() {
  if (el.zoomVal) el.zoomVal.textContent = Math.round(state.view.k * 100) + ' %';
}
...
function updateStatus() {
  ...
  var hid = Object.keys(state.hidden).length;
...
function openHelp() { el.hp.classList.add('show'); }
function closeHelp() { el.hp.classList.remove('show'); }

document.getElementById('btn-help').addEventListener('click', openHelp);
document.getElementById('hp-close').addEventListener('click', closeHelp);
el.hp.addEventListener('mousedown', function (ev) { ev.stopPropagation(); });

document.getElementById('btn-zoom-in').addEventListener('click', function () { zoomBy(1.15); });
document.getElementById('btn-zoom-out').addEventListener('click', function () { zoomBy(1 / 1.15); });
el.zoomVal.addEventListener('click', function () { fitToScreen(); updateZoomLabel(); });

el.btnHide.addEventListener('click', hideSelected);
el.btnHidden.addEventListener('click', openHiddenBox);
document.getElementById('hb-close').addEventListener('click', closeHiddenBox);
el.hb.addEventListener('mousedown', function (ev) { ev.stopPropagation(); });

document.getElementById('btn-card').addEventListener('click', function () { openNewCard(null); });
document.getElementById('te-tocard').addEventListener('click', function () {
  if (!teNode) return;
  var txt = el.teText.value.trim();
  if (!txt) { el.teText.focus(); return; }
  teNode.ref.text = txt;
  openNewCard(teNode);
});
document.getElementById('nc-close').addEventListener('click', closeNewCard);
document.getElementById('nc-create').addEventListener('click', createCard);
el.nc.addEventListener('mousedown', function (ev) { ev.stopPropagation(); });
el.ncName.addEventListener('keydown', function (ev) {
  if (ev.key === 'Enter') { ev.preventDefault(); createCard(); }
});
...
el.btnDelete.addEventListener('click', function () {
  ...
  state.edges = state.edges.filter(function (e) {
    return ids.indexOf(e.f) < 0 && ids.indexOf(e.t) < 0;
  });
  if (state.edges.length !== before) { render(); updateStatus(); saveEdges(); }
});
