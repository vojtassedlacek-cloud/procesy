...
function render() {
  ...
  state.edges.forEach(function (e) {
    if (state.hidden[e.f] || state.hidden[e.t]) return;
    drawEdge(e);
  });
  state.nodes.forEach(function (n) {
...
(function initPointer() {
  ...
  window.addEventListener('mousemove', function (e) {
    ...
    if (state.drag) {
      ...
      state.edges.forEach(function (ed) {
        if (state.hidden[ed.f] || state.hidden[ed.t]) return;
        drawEdge(ed);
      });
      positionPalette();
      return;
  ...
  window.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') {
      clearLink(); closeCardEditor(); closeTextEditor(); closeNewCard(); closeHiddenBox();
    }
