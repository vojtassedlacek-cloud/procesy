...
function saveMapData() {
  ...
  var data = {
    ...
    t: state.texts.map(function (x) {
      return { i: x.id, x: x.x, y: x.y, s: x.text, hd: x.heading ? 1 : 0 };
    }),
...
function openMap(id) {
  ...
  return loadMapData(id).then(function (data) {
    state.texts = (data.t || []).map(function (x) {
      return { id: x.i, x: x.x, y: x.y, text: x.s || '', heading: !!x.hd };
    });
...
function drawFrame(f) {
  ...
  var grip = svgEl('rect', {
    'class': 'frame-resize', x: f.w - 9, y: f.h - 9, width: 18, height: 18, rx: 4, ry: 4
  });
  grip.addEventListener('mousedown', function (ev) {
    ev.stopPropagation();
    ev.preventDefault();
    state.selectedFrame = f.id;
    state.frameDrag = { frame: f, mode: 'resize', start: toCanvas(ev), w0: f.w, h0: f.h };
  });
  g.appendChild(grip);
...
function rebuildTextNodes() {
  ...
  state.texts.forEach(function (x) {
    ...
    var lines = wrapText(x.text || 'Klikni dvakrát a piš…', 30, 8);
    state.nodes.push({
      id: x.id, kind: 'text', heading: false, listId: null, name: x.text, card: null, ref: x,
      lines: lines, top: 16, hasLabels: false,
      h: 16 + lines.length * LINE_H + PAD_BOT, x: x.x, y: x.y
    });
  });
  state.nodeById = {};
  state.nodes.forEach(function (n) { state.nodeById[n.id] = n; });
...
(function initPointer() {
  ...
  window.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') { clearLink(); closeCardEditor(); }
