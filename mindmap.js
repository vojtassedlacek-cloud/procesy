...
function loadData() {
  ...
  return Promise.all([
    t.board('id', 'name'),
    t.lists('id', 'name'),
    t.cards('id', 'name', 'idList', 'shortLink', 'url', 'labels', 'checklists', 'closed', 'due', 'dueComplete')
  ]).then(function (res) {
    state.board = res[0];
    state.lists = res[1] || [];
    ...
    render();
    setStatus(plural(state.cards.length, 'karta', 'karty', 'karet') + ' · ' +
              plural(state.lists.length, 'seznam', 'seznamy', 'seznamů'));
  }).catch(function (err) {
    console.error(err);
...
function onNodeClick(n, ev) {
  ...
  var items = [{
    text: 'Otevřít kartu v nové záložce',
...
function openCard(card) {
  ...
  try {
    ...
    t.alert({ message: 'Kartu se nepodařilo otevřít.', duration: 5 });
  }
}
...
function plural(n, one, few, many) {
  var w = n === 1 ? one : (n >= 2 && n <= 4 ? few : many);
  return n + ' ' + w;
}
